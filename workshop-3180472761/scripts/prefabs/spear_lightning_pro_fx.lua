local asset_pink_fx = {Asset("ANIM", "anim/swap_spear_lightning_pink.zip")}
...
local function StartFX(proxy, animindex, build)
    ...
    if not TheNet:IsDedicated() then inst.entity:AddSoundEmitter() end
...
local function OnUpdateFlash(inst)
    ...
    elseif inst.flash > .1 then
        ...
        local c = inst.flash *
                      (inst.blink < 2 and inst.intensity or inst.intensity * .25)
...
local function MakeSparks(name, build)
    ...
    local function OnRandDirty(inst)
        if inst._complete or inst._rand:value() <= 0 then return end
...
local function FxFn()
    ...
    inst.AnimState:SetBank("swap_spear_lightning_pink")
    inst.AnimState:SetBuild("swap_spear_lightning_pink")
    ...
    -- inst.AnimState:HideSymbol("swap_spear_lightning_pink")
...
return MakeSparks("spear_lightning_pink_attack_fx",
                  "spear_lightning_pink_attack_fx"),
       Prefab("spear_lightning_pink_fx", FxFn, asset_pink_fx)
