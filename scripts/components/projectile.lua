...
local ProjectileShield = Class(function(self, inst)
    self.inst = inst
    self.owner = nil
    self.target = nil
    self.start = nil
...
function ProjectileShield:OnRemoveFromEntity()
...
function ProjectileShield:GetDebugString()
    return string.format("target: %s, owner %s", tostring(self.target), tostring(self.owner))
end

function ProjectileShield:SetSpeed(speed)
    self.speed = speed
...
function ProjectileShield:SetStimuli(stimuli)
    self.stimuli = stimuli
end

function ProjectileShield:SetRange(range)
    self.range = range
end

function ProjectileShield:SetHitDist(dist)
    self.hitdist = dist
end

function ProjectileShield:SetOnThrownFn(fn)
    self.onthrown = fn
end

function ProjectileShield:SetOnHitFn(fn)
    self.onhit = fn
end

function ProjectileShield:SetOnPreHitFn(fn)
    self.onprehit = fn
end

function ProjectileShield:SetOnCaughtFn(fn)
    self.oncaught = fn
end

function ProjectileShield:SetOnMissFn(fn)
    self.onmiss = fn
end

function ProjectileShield:SetCanCatch(cancatch)
    self.cancatch = cancatch
end

function ProjectileShield:SetHoming(homing)
    self.homing = homing
end

function ProjectileShield:SetLaunchOffset(offset)
    self.launchoffset = offset -- x is radius, y is height, z is ignored
end

function ProjectileShield:IsThrown()
    return self.target ~= nil
end

function ProjectileShield:Throw(owner, target, attacker)
    self.owner = owner
    self.target = target
...
function ProjectileShield:Catch(catcher)
...
function ProjectileShield:Miss(target)
...
function ProjectileShield:Stop()
...
function ProjectileShield:Hit(target)
    local attacker = self.owner
...
function ProjectileShield:DelayOffscreenMiss_Internal()
	if self.dozeOffTask == nil then
		self.dozeOffTask = self.inst:DoTaskInTime(DOZE_OFF_TIME, DozeOff, self)
	end
end

function ProjectileShield:OnEntitySleep()
	if self.target then
		self:DelayOffscreenMiss_Internal()
	end
end

function ProjectileShield:OnEntityWake()
    if self.dozeOffTask ~= nil then
        self.dozeOffTask:Cancel()
        self.dozeOffTask = nil
    end
...
function ProjectileShield:OnUpdate(dt)
...
function ProjectileShield:RotateToTarget(dest)
...
function ProjectileShield:DelayVisibility(duration)
    if self.delaytask ~= nil then
        self.delaytask:Cancel()
    end
    self.inst:Hide()
...
function ProjectileShield:SetBounced(bounced)
	self.bounced = bounced ~= false
end

function ProjectileShield:IsBounced()
	return self.bounced == true
end

return ProjectileShield
