local song_tunings = require("prefabs/new_battlesongdefs").song_defs
...
local function song_fn(songdata, prefabname)
    ...
    inst.AnimState:SetBank("new_battlesongs")
    inst.AnimState:SetBuild("new_battlesongs")
    ...
    inst.components.inventoryitem.imagename = prefabname .. "_unavaliable"
    inst.components.inventoryitem.atlasname = "images/inventoryimages/new_battlesongs.xml"
    ...
    if inst.songdata.LOOP_FX and target.battlesong_fx_task == nil and
        ...
        target.battlesong_fx_task = inst:DoPeriodicTask(4 + math.random(), function()
            ...
            if target.components.singinginspiration ~= nil and target.components.singinginspiration:IsSinging() then
                ...
                SpawnFx("battlesong_loop", target, 0.4, GetHorizontalOffset(), 1.6 + math.random() / 5,
                    GetHorizontalOffset())
            end
        end)
    end
...
local function buff_OnAttached(inst, target)
    ...
    inst.task = inst:DoPeriodicTask(TUNING.SONG_REFRESH_PERIOD, buff_OnTick, TUNING.SONG_REFRESH_PERIOD + math.random(),
        target)
...
local function AddSong(songs, name, songdata)
    local assets = {
        Asset("ANIM", "anim/new_battlesongs.zip"),
        Asset("ATLAS", "images/inventoryimages/new_battlesongs.xml"),
        Asset("SCRIPT", "scripts/prefabs/new_battlesongdefs.lua"),
    }
